...
lp_obj lpf_bind(LP) {
	...
	lp_fnc* fnc = lp_obj_to_fnc(r);
    return lp_fnc_new(lp,
        fnc->ftype|2,fnc->cfnc,fnc->code,
        self,fnc->globals);
}
...
lp_obj lpf_copy(LP) {
    ...
    lp_raise(0,lp_string_new(lp, "(lp_copy) TypeError: ?"));
}
...
lp_obj lpf_istype(LP) {
    lp_obj v = LP_OBJ(0);
    lp_obj t = LP_STR(1);
    if (lp_cmp(lp, t, lp_string_new(lp, "str")) == 0) { return lp_number_from_int(lp, (v & 0x7) == LP_STRING); }
    if (lp_cmp(lp, t, lp_string_new(lp, "list")) == 0) { return lp_number_from_int(lp, (v & 0x7) == LP_LIST); }
    if (lp_cmp(lp, t, lp_string_new(lp, "dict")) == 0) { return lp_number_from_int(lp, (v & 0x7) == LP_DICT); }
...
lp_obj lpf_load(LP) {
    ...
    f = fopen(fname,"rb");
    if (!f) {
        lp_raise(0,lp_string_new(lp, "(lp_load) IOError: ?"));
    }
    r = lp_string_t(lp,l);
    s = r->string.info->s;
    fread(s,1,l,f);
